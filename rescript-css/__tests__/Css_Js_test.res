...
describe("lineHeight", () => {
  test("test usage", () => {
    ...
    )->toBe({"lineHeight": "calc(calc(100% - 20px) + calc(1vw / 2))"})
    expect(lineHeight(var("--foo")))->toBe({"lineHeight": "var(--foo)"})
  })
})
...
describe("Filter", () =>
  test("test values", () => {
    expect(filter([#opacity(10.), #invert(20.)]))->toBe({"filter": "opacity(10%) invert(20%)"})
    expect(filter([#blur(#px(20)), #brightness(20.)]))->toBe({
      "filter": "blur(20px) brightness(20%)",
    })
    expect(
      filter([#contrast(30.), #dropShadow((#px(5), #px(6), #px(7), #rgb((255, 0, 0))))]),
    )->toBe({
      "filter": "contrast(30%) drop-shadow(5px 6px 7px rgb(255, 0, 0))",
    })
    expect(filter([#grayscale(10.), #hueRotate(#deg(180.))]))->toBe({
      "filter": "grayscale(10%) hue-rotate(180deg)",
    })
    expect(filter([#saturate(10.), #sepia(100.)]))->toBe({"filter": "saturate(10%) sepia(100%)"})
    expect(filter([#none]))->toBe({"filter": "none"})
...
describe("Backdrop filter", () =>
  test("test values", () => {
    expect(backdropFilter([#none]))->toBe({"backdropFilter": "none"})
    expect(backdropFilter([#blur(#px(10)), #brightness(#percent(42.0))]))->toBe({
      "backdropFilter": "blur(10px), brightness(42%)",
    })
    expect(backdropFilter([#contrast(#num(10)), #dropShadow(#percent(0.5))]))->toBe({
      "backdropFilter": "contrast(10), drop-shadow(0.5%)",
    })
    expect(backdropFilter([#grayscale(#percent(99.9)), #hueRotate(#deg(90.0))]))->toBe({
      "backdropFilter": "grayscale(99.9%), hue-rotate(90deg)",
    })
    expect(backdropFilter([#invert(#num(30)), #opacity(#percent(10.0))]))->toBe({
      "backdropFilter": "invert(30), opacity(10%)",
    })
    expect(backdropFilter([#saturate(#num(30)), #sepia(#percent(10.0))]))->toBe({
...
describe("box-shadow", () => {
  test("should allow single or list definition", () => {
    ...
    expect(boxShadows([Shadow.box(yellow), Shadow.box(red)]))->toBe({
...
describe("text-shadow", () => {
  test("should allow single or list definition", () => {
    ...
    expect(textShadows([Shadow.text(yellow), Shadow.text(red)]))->toBe({
...
describe("transitions", () => {
  test("should allow single or list definition", () => {
    ...
    expect(transitions([Transition.shorthand("height"), Transition.shorthand("top")]))->toBe({
...
describe("gridTemplateColumns", () => {
  ...
  test("unfolds repeats", () => {
    expect(gridTemplateColumns([#repeat((#num(4), #fr(1.)))]))->toBe({
      "gridTemplateColumns": "repeat(4, 1fr)",
    })
    expect(gridTemplateColumns([#repeat((#num(4), #auto))]))->toBe({
      "gridTemplateColumns": "repeat(4, auto)",
    })
    expect(gridTemplateColumns([#repeat((#num(4), #minContent))]))->toBe({
      "gridTemplateColumns": "repeat(4, min-content)",
    })
    expect(gridTemplateColumns([#repeat((#num(4), #maxContent))]))->toBe({
...
describe("backgroundPosition", () => {
  ...
  test("test multiple positions", () => {
    expect(backgroundPositions([#hv((px(0), px(0))), center]))->toBe({
